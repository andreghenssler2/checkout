...
?><!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title><?= Support::e(SiteSettings::pageTitle('Entrar')) ?></title><?php SiteSettings::renderFavicon(); ?><link rel="stylesheet" href="<?= APP_URL ?>/assets/css/app.css?v=1.8.9"></head><body class="bg"><main class="login-card"><div class="panel"><h1><?= Support::e(SiteSettings::title()) ?></h1><p>Administração</p><?php if($error):?><div class="alert error"><?= Support::e($error) ?></div><?php endif;?><form method="post"><input type="hidden" name="_csrf" value="<?= Support::csrf() ?>"><label>E-mail<input type="email" name="email" autocomplete="username" required></label><label>Senha<input type="password" name="senha" autocomplete="current-password" required></label><button class="btn primary fullbtn">Entrar</button></form></div></main></body></html>
